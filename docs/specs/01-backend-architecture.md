...
Fronteira HTTP — recebe requisições, delega para services, converte e retorna respostas. Sem regras de negócio.

service

Lógica da aplicação e orquestração do domínio. Independe de conceitos HTTP. Contém todas as validações de negócio.

repository

Acesso exclusivo a dados (JPA/Hibernate). Apenas queries e persistência.

model

Entidades de domínio — Session, Message, Attachment. Podem utilizar anotações JPA, mas não devem conter responsabilidades HTTP ou regras de orquestração da aplicação.

dto

Contratos de entrada e saída da API. Imutáveis, sem lógica de negócio.

config

Configurações da aplicação (CORS, upload size, IA strategy).

exception

Tratamento global de erros com @ControllerAdvice e ProblemDetail (RFC 9457).
...
Título opcional da sessão (padrão 'Nova sessão' quando ausente/vazio, retornado sempre não nulo no DTO)
...
Payload de entrada do POST /api/chat — sessionId, content

ChatResponse

Payload de saída do POST /api/chat — userMessage (MessageDTO), assistantMessage (MessageDTO)

SessionDTO

Representação de uma sessão — id, title (não nulo), createdAt, updatedAt

CreateSessionRequest

Payload de criação de sessão — title (opcional, padrão 'Nova sessão')

MessageDTO

Representação de uma mensagem — id, sessionId, sender, content, timestamp

AttachmentDTO

Representação de um anexo — id, sessionId, filename, type, size, uploadDate

HealthDTO

Resposta do health check — status, timestamp
...
POST /api/upload, GET /api/sessions/{id}/attachments (opcional Etapa 1)
...
Criar, listar e buscar sessões. Listagem ordenada por updatedAt decrescente. Oferece touchSession(Long id) para atualizar updatedAt da sessão — chamado por ChatService e AttachmentService. Quando title é ausente/vazio em createSession, define como "Nova sessão".

MessageService

Persistir mensagens e recuperar histórico por sessão ordenado por timestamp crescente.

ChatService

Orquestrar recebimento de mensagem, validar existência da sessão, persistir mensagens, atualizar updatedAt da sessão via SessionService.touchSession() e gerar resposta simulada.

AttachmentService

Processar upload, validar tipo/extensão/tamanho, validar existência da sessão, salvar arquivo e metadados, atualizar updatedAt da sessão via SessionService.touchSession(). Não recebe MultipartFile diretamente — recebe nome, tipo MIME, tamanho, conteúdo do arquivo e sessionId.

AiResponseGenerator

Interface para geração de respostas (mock agora, IA real no futuro).
...
{ "status": "UP", "timestamp": "2026-06-26T10:00:00Z" }
...
[SessionDTO] (ordenado por updatedAt desc)
...
{ "userMessage": MessageDTO, "assistantMessage": MessageDTO }
...
[MessageDTO] (ordenado por timestamp asc)
...
file (TXT/PDF), sessionId (obrigatório)
...
Listar anexos de uma sessão (opcional Etapa 1)
...
Usuário clica em "Nova sessão" no front-end

Front-end envia POST /api/sessions com title opcional

SessionController recebe a requisição, converte para CreateSessionRequest e delega para SessionService

SessionService cria entidade Session com createdAt e updatedAt

SessionRepository persiste a entidade no banco

SessionController retorna SessionDTO com status 201

Front-end atualiza a lista de sessões

13. Fluxo de envio de mensagem

Usuário digita mensagem e pressiona Enter no front-end

Front-end envia POST /api/chat com { "sessionId": 1, "content": "..." }

ChatController recebe a requisição, converte para ChatRequest e delega para ChatService

ChatService valida existência da sessão via SessionService

ChatService persiste a mensagem do usuário via MessageService

ChatService invoca AiResponseGenerator para obter resposta simulada

Resposta simulada é persistida como mensagem do assistente via MessageService

ChatController retorna { "userMessage": MessageDTO, "assistantMessage": MessageDTO } com status 201

Front-end exibe ambas as mensagens no chat

14. Fluxo de recuperação do histórico

Usuário seleciona uma sessão no front-end

Front-end envia GET /api/sessions/{id}/messages

MessageController recebe a requisição e delega para MessageService

MessageService valida existência da sessão via SessionService

MessageService busca mensagens ordenadas por timestamp ascendente

MessageController retorna lista de MessageDTO com status 200

Front-end renderiza as mensagens no chat

15. Fluxo de upload de documentos

Usuário arrasta arquivo ou clica para selecionar no front-end

Front-end envia POST /api/upload com multipart/form-data contendo file e sessionId (obrigatório)

AttachmentController recebe o MultipartFile e extrai nome, tipo MIME, tamanho, conteúdo e sessionId, convertendo para um objeto de entrada sem dependência HTTP; delega para AttachmentService

AttachmentService recebe nome, tipo MIME, tamanho, conteúdo do arquivo e sessionId; valida existência da sessão via SessionService

AttachmentService valida extensão (.txt, .pdf), tipo MIME e tamanho máximo (10 MB)

Arquivo é salvo no sistema de arquivos (diretório configurável)

AttachmentService persiste metadados via AttachmentRepository

AttachmentController retorna AttachmentDTO com status 201

Em caso de arquivo inválido ou sessão inexistente, retorna 400 ou 404 com ProblemDetail

Front-end exibe progresso durante o upload e confirmação ao final
